...
beta = [1.6 1.7];
RMS = [1];
angled=[0 30 -30 45 -45 60 -60 70 -70 75 -75 80 -80];
...
for ibeta=1:length(beta)
    for irms=1:length(RMS)
        ...
        outdir=fullfile('exp1b_Varor',['Beta_',num2str(beta(ibeta)),'RMS_',num2str(RMS(irms))]);
        if ~exist(outdir, 'file')
            disp(['creando ' outdir])
            try
                mkdir(outdir);
            catch
                disp(['non podo crear  ' outdir]);
            end
        end


        for iang=1:length(angled)

            %Aleatorizamos a posicion do circulo
            r = ceil(200.*rand(1,2))-100;
            x = 256-r(1); y = 256-r(2);

            T = GenerateFractalHeightMap(N, beta(ibeta), RMS(irms), seed);
            D = GenerateEllipsoidDefect(a, b, c);
            D=imrotate(D,-(angled(iang)+90),'bicubic');
            [T mask] = AddDefect2Surface(T, D, vol, x, y, c);
            R = lambertian(T, slant, tilt, N);
            R = (R>0).*R;


            imwrite(R, fullfile(outdir,[num2str(angled(iang)), '.png']));
            imwrite(mask, fullfile(outdir,['mask_',num2str(angled(iang)), '.png']));
        end
