function [I,xcent,ycent,xfound,yfound]=image_popout_flanking_xy(gridsize,len,wid,spacing,angle,contrast,backgnd,jitter_pos,jitter_ang,surr_prob,acent,bcent,rconj,sconj,xfound,yfound)
...
if nargin<10, surr_prob=[1 0.5]; end
...
if nargin<13, rconj=getsconj( gridsize, surr_prob(end) ); end
if nargin<14, sconj=getsconj( gridsize, surr_prob(1) ); end
if nargin<15, [ xfound,yfound ] = getjitterpos( imsize, hlen, xlocations, ylocations, jitter_pos, acent,bcent,surr_prob(1),sconj); end
...
for x=xlocations
  ...
  for y=ylocations
    b=b+1;
    if sconj(a,b)<=surr_prob(1) || (a==acent && b==bcent)
        coo=coo+1;
        xval=xfound(coo);
        yval=yfound(coo);
    end
    angleval1=(angle(2)-angle(3))+(2*jitter_ang*rand)-jitter_ang;
    angleval2=(angle(2)-angle(4))+(2*jitter_ang*rand)-jitter_ang;
    if a==acent && b==bcent
      ...
      if sconj(a,b)<=surr_prob(1)
        if rconj(a,b)<=surr_prob(2)
            I=draw_bar(I,xval,yval,bar,angleval1,contrast(2),backgnd);
        else
